document.addEventListener('DOMContentLoaded', () => {
    ...
    // Send message when the user presses Enter in the message input field
$("#user_message").keypress(function(event) {
    // Check if the pressed key is Enter
    if (event.which === 13) {
        event.preventDefault(); // Prevent the default Enter behavior (e.g., adding a line break)
        sendMessage();
    }
});
        ...
        function sendMessage(recipientId, message) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    fetchMessages(recipientId);
                    $("#user_message").val("");
                    sendingMessage = false; // Reset flag after successful send
                },
                error: function(xhr, status, error) {
